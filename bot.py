...
CONFIG = {
    ...
    'MAX_CONCURRENCY': 10  # Jumlah pendaftaran paralel maksimum
}

# ABI minimal untuk kontrak controller
CONTROLLER_ABI = [
    # ... (ABI tetap sama, tidak perlu diubah) ...
    {
...
def load_file_lines(filename: str) -> List[str]:
    try:
        with open(filename, 'r') as f:
            # Membaca semua baris, menghapus spasi, dan mengubah ke huruf kecil
...
def test_proxy(proxy: str) -> bool:
    """Menguji apakah proxy berfungsi atau tidak"""
    try:
...
def validate_private_key(private_key: str) -> bool:
    """Memeriksa apakah kunci privat valid"""
...
def register_domain(private_key: str, name: str, task_index: int, proxy: str = None) -> None:
    ...
    while retry < MAX_RETRY:
        try:
            ...
            # 2. Mengirim commit
            tx = controller.functions.commit(commitment).build_transaction({'from': owner, 'nonce': w3.eth.get_transaction_count(owner), 'gas': 200000, 'gasPrice': w3.eth.gas_price})
            signed_tx = w3.eth.account.sign_transaction(tx, private_key)
            tx_hash = w3.eth.send_raw_transaction(signed_tx.raw_transaction)
            w3.eth.wait_for_transaction_receipt(tx_hash)
            logger.info(f"[Tugas #{task_index}] Commitment telah terkirim untuk {name}.phrs!")

            # 3. Menunggu minCommitmentAge
            logger.info(f"[Tugas #{task_index}] Menunggu minCommitmentAge selama 60 detik...")
            time.sleep(60)
            ...
            # 5. Mendaftar
            tx = controller.functions.register(name, owner, CONFIG['DURATION'], HexBytes(secret), resolver_address, CONFIG['DATA'], CONFIG['REVERSE_RECORD'], CONFIG['OWNER_CONTROLLED_FUSES']).build_transaction({'from': owner, 'nonce': w3.eth.get_transaction_count(owner), 'gas': 300000, 'gasPrice': w3.eth.gas_price, 'value': value})
            signed_tx = w3.eth.account.sign_transaction(tx, private_key)
            tx_hash = w3.eth.send_raw_transaction(signed_tx.raw_transaction)
            w3.eth.wait_for_transaction_receipt(tx_hash)
            logger.info(f"✅ [Tugas #{task_index}] Pendaftaran {name}.phrs berhasil!")
            break
            ...
            if retry < MAX_RETRY:
                logger.warning(f"[Tugas #{task_index}] Error pada {name}.phrs: {msg} - menunggu 60 detik sebelum mencoba lagi {retry}/{MAX_RETRY}...")
                time.sleep(60)
            else:
                logger.error(f"❌ [Tugas #{task_index}] Gagal mendaftarkan {name}.phrs setelah {MAX_RETRY} kali percobaan: {msg}")
                break

def main():
    pk_list = load_file_lines("pk.txt")
    domain_list = load_file_lines("domain.txt") # <-- BARU: Memuat daftar domain
    proxy_list = [proxy for proxy in load_file_lines("proxy.txt") if test_proxy(proxy)]
    
    if not pk_list:
        logger.error("Kunci privat tidak ditemukan di dalam pk.txt. Bot berhenti.")
        return
        
    if not domain_list:
        logger.error("Nama domain tidak ditemukan di dalam domain.txt. Bot berhenti.")
        return

    # Membuat daftar tugas dari domain yang ada di domain.txt
    # Setiap domain akan didaftarkan oleh satu wallet secara bergiliran (round-robin)
    tasks = []
    ...
    logger.info(f"Siap mendaftarkan {len(tasks)} domain menggunakan {len(pk_list)} wallet...")

    with ThreadPoolExecutor(max_workers=CONFIG['MAX_CONCURRENCY']) as executor:
        futures = [executor.submit(register_domain, pk, name, task_idx, random.choice(proxy_list) if proxy_list else None)
                   for pk, name, task_idx in tasks]
        for future in futures:
            future.result()  # Menunggu semua tugas selesai
...
if __name__ == "__main__":
    while True:
        try:
            ...
            logger.error(f"Error fatal di dalam main: {str(err)}")
            logger.info("Menunggu 60 detik sebelum mencoba lagi semuanya...")
            time.sleep(60)
